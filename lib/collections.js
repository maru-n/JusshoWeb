...
var LAMBDA_RESULT_WAITING_TIME = 18000
...
Photos.insertFileBelongedOperation = function(file, targetOperationId, callback) {
    async.waterfall([
        ...
        function checkLambdaResult(photoId, next) {
            var handle = Photos.find(photoId).observeChanges({
                changed: function (photoId, fields) {
                    if (fields.available) {
                        clearTimeout(timer);
                        handle.stop();
                        next(null, photoId)
        ...
        }], function(error, photoId) {
            callback(error, photoId);
        });
}
